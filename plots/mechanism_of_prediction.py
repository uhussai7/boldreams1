...
def row_(model_config,vox_id,img_id,roi_filter,ax,ax_ind):
    # set up the encoder
    ...
    print('predicted:',enc(img)[:,roi_filter][:,vox_id])
    print( 'true:',p_enc.train_data.dataset.tensors[1][img_id,roi_filter][vox_id])
...
p_enc = prep_encoder(load_config(base+'/configs/alexnet_medium_bbtrain-off_max-channels-100.json'), df)
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
enc = p_enc.get_encoder(device)
